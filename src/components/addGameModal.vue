  ...
  <div class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header text-white bg-secondary justify-content-start"><i-fa-solid:user-plus class="me-1" />Add a Game</div>
        <div class="modal-body">
          <form ref="form" @submit.prevent="onSubmit" id="gameForm" @input="formValid = form.checkValidity()">
            <fieldset :disabled="appState.requestPending">
              <div class="form-check">
                ...
                <input required type="text" id="otherTeamName" v-model="gameTemp.other_team" class="form-control" />
              </div>
            </fieldset>
          </form>
        </div>
        <div class="modal-footer bg-light">
          <button :disabled="!formValid || appState.requestPending" class="btn btn-primary" form="gameForm" type="submit">
            <div class="spinner-border spinner-border-sm" v-if="appState.requestPending == `POST /games/new`" />
            Create game
          </button>
...
async function onSubmit() {
  ...
  const newGame = await apiCall(`/games/new`, {
    method: "POST",
